"""
`FastNetMixin._extract_fits` builds the dense coefficient path in place in
the solver's `ca` output buffer instead of allocating a second copy. These
tests check it against the previous (allocate-and-scatter) implementation
and that the memory saving is real. No R needed.
"""
import subprocess
import sys
import textwrap
...
def fit_with_raw(cls, X, y, **kwargs):
    """Fit, and also capture the solver's raw outputs as `_extract_fits` saw them."""
    raw = {}
...
def previous_extract(raw, n_features):
    """The allocate-and-scatter extraction this replaced (single response)."""
    nfits = raw['lmu']
...
def check_against_previous(L, raw, n_features):
    ...
    # the solver buffer is consumed, not kept alongside coefs_
...
def test_early_stop_uses_only_fitted_lambdas():
    # n > p with a strong signal: the path stops on devmax before nlambda
...
def test_df_max_shrinks_buffer(sparse):
    # nx = min(2 * df_max + 20, p) < p: the buffer cannot hold the dense
    # result, so the old allocation path is used
    n, p, df_max = 300, 100, 5
    X, y = make_data(n, p, sparse=sparse)
    L, raw = fit_with_raw(GaussNet, X, y, nlambda=30, df_max=df_max)
    assert raw['nx'] == 2 * df_max + 20 < p
    check_against_previous(L, raw, p)
    # and agrees with the unrestricted path on the lambdas both fitted
    full = GaussNet(nlambda=30).fit(X, y)
    k = L.coefs_.shape[0]
    np.testing.assert_allclose(L.coefs_, full.coefs_[:k], rtol=1e-6, atol=1e-10)


def test_lognet_df_max_shrinks_buffer():
    # lognet's buffer is a flat vector with one block of nx per lambda; the
    # previous code reshaped it with blocks of n_features, which was wrong
    # whenever df_max shrank nx
    n, p, df_max = 300, 100, 5
    X, y = make_data(n, p)
    y = (y > 0).astype(float)
    L, raw = fit_with_raw(LogNet, X, y, nlambda=30, df_max=df_max)
    assert raw['nx'] == 2 * df_max + 20 < p
    check_against_previous(L, raw, p)
...
def test_peak_memory_is_about_one_copy():
    # peak RSS of a 200k-feature, 100-lambda sparse fit relative to the
    # (nlambda, n_features) float64 path: ~3.2 before, ~1.2 now
